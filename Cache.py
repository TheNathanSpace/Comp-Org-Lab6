...
class Cache:
    def __init__(self, num_sets: int, blocks_per_set: int, replacement_policy: int):
    ...
    def get_split_address(self, bin_address: list):
        split_address = {}
        split_address["tag"] = self.bin_to_dec(bin_address[slice(0, 18)])
        split_address["index"] = self.bin_to_dec(bin_address[slice(18, 28)])
        split_address["offset"] = self.bin_to_dec(bin_address[slice(28, 32)])
